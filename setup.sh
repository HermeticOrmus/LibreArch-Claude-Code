...
for name in "${SELECTED[@]}"; do
  src="$PLUGINS_SRC/$name"; dst="$PLUGINS_DST/libre-arch-$name"
  [[ ! -d "$src" ]] && { echo "  [skip] $name"; continue; }
  [[ -d "$dst" ]] && { echo "  [skip] libre-arch-$name (installed)"; continue; }
  cp -r "$src" "$dst"; echo "  [ok] libre-arch-$name"; count=$((count + 1))
done
